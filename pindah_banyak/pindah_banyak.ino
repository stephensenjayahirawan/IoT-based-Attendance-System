...
//const char* wifiName = "Tenda_06F090";
//const char* wifiPass = "12345678";

//const char* wifiName = "it@bkaunpar";
//const char* wifiPass = "kantinqiu";
 
const char* wifiName     = "OnePlus3t";
const char* wifiPass = "parahyangan";
...

{
  ...
  if (status_fingerprint == FINGERPRINT_OK) {
...
//  Serial.println(payload);    //Print request response payload
  ...
  if(httpCode == 200)
  {
   JsonObject& root = jsonBuffer.parseObject(payload);
    ...
    Serial.println(root["jumlah_fingerprint"].as<int>());
    
    uint8_t id = 0;
    for( int j = 0; j < root["jumlah_fingerprint"].as<int>(); j++){
      Serial.print(j);
        Serial.println("------------------------------------");
    ...
    //    mySerial.write(fingerprint1,sizeof(fingerprint1)); 
        mySerial.flush();
        uint8_t status_fingerprint = finger.uploadTemplate();
        mySerial.flush();
        if (status_fingerprint == FINGERPRINT_OK) {
          Serial.println("Writing Template!");
          for(int i = 0; i < root["FINGERPRINT"][j]["length"].as<int>()-1; i++){
              byte a = byte(root["FINGERPRINT"][j]["template"][i].as<int>());
              mySerial.write(a);
              yield();
          }
          mySerial.flush();
        } else if (status_fingerprint == FINGERPRINT_PACKETRECIEVEERR) {
          Serial.println("Communication error");
        } else if (status_fingerprint == FINGERPRINT_BADLOCATION) {
          Serial.println("Could not store in that location");
        } else if (status_fingerprint == FINGERPRINT_FLASHERR) {
          Serial.println("Error writing to flash");
        } else {
          Serial.println("Unknown error");
        }  
        
//        status_fingerprint = finger.uploadTemplate2();
//          mySerial.flush();
//          
//        if (status_fingerprint == FINGERPRINT_OK) {
//          Serial.println("Writing Template!");
...
//        }  
         mySerial.flush();
        status_fingerprint = finger.storeModel(++id);
        NPM[id] = root["FINGERPRINT"][j]["NPM"].as<String>();
...
//          id++;
//          status_fingerprint = finger.storeModel2(id);
//          NPM[id] = root["FINGERPRINT"][j]["NPM"].as<String>();
//     
//          mySerial.flush();
//        if (status_fingerprint == FINGERPRINT_OK) {
...
//   http.end();
  ...
  if (status_fingerprint != FINGERPRINT_OK)  return -1;
  ...
  Serial.print("with NPM :"); Serial.print(NPM[finger.fingerID/2+1]); 
