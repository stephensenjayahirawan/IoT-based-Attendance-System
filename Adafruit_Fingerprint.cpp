...
uint8_t Adafruit_Fingerprint::storeModel2(uint16_t location) {
  SEND_CMD_PACKET(FINGERPRINT_STORE, 0x02, (uint8_t)(location >> 8), (uint8_t)(location & 0xFF));
}
uint8_t Adafruit_Fingerprint::uploadTemplate (void) {
  SEND_CMD_PACKET(FINGERPRINT_DOWNLOAD, 0x01);
}
uint8_t Adafruit_Fingerprint::uploadTemplate2 (void) {
